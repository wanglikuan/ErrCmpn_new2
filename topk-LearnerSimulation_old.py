...
def get_upload(g_remain, g_new, ratio, isCompensate, g_threshold):
    ...
    # threshold
    param_num = torch.numel(g_change_merge)      #zheli
    # threshold = ratio * g_threshold / np.sqrt(param_num)

    the_k_idx = int(ratio*param_num)
    the_k_idx = the_k_idx if the_k_idx > 0 else 1
    #
    """
    for idx, g_layer in enumerate(g_change):
        if idx == 0:
            the_k_threshold = g_layer
        if idx <= the_k_idx:
            if g_layer < the_k_threshold:
                the_k_threshold = g_layer
        else:
            if g_layer > the_k_threshold:
                the_k_threshold = g_layer

    threshold = the_k_threshold
    """
    top_k = torch.topk(g_change_merge, the_k_idx)
    threshold = top_k[0][the_k_idx-1].item()
    ...
    for idx, g_layer in enumerate(g_change):
        ...
        g_change_tmp[mask] = 0.0 # ba <threshold de zhi ji wei 0
...
def run(workers, models, save_path, train_data_list, test_data, iterations_epoch):
    ...
    trainloss_file = './trainloss_topk_oldsimu' + args.model + '_transform_w10r0.1lr0.1.txt'
    ...
    for iteration in range(args.epochs * iterations_epoch):
        ...
        for i in workers:        # zheli
            ...
            if global_clock == 1:        # chushihua diyilun
                ...
                g_remain, g_large_change, sparsification_ratio= get_upload(g_remain_list[i-1],delta_ws, ratio,args.isCompensate, threshold) #hanshu
                g_remain_list[i-1] = g_remain  # server g gengxin
                # synchronous update
                for g_change_layer_idx, g_change_layer in enumerate(g_change_average):  # line5
                    g_change_layer.data += g_large_change[g_change_layer_idx].data/workers_num #line5


        # 同步操作
        g_quare_sum = 0.0   # for threshold     server
        for p_idx, param in enumerate(models[0].parameters()):
            global_g[p_idx].data += g_change_average[p_idx].data   #zheli   delta pingjun
            param.data -= global_g[p_idx].data
            for w in workers:    #gengxin w
                list(models[w].parameters())[p_idx].data = param.data + torch.zeros_like(param.data)  #hui chuan

            g_quare_sum += torch.sum(global_g[p_idx].data * global_g[p_idx].data)    #server    buxishuo
